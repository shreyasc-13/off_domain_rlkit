...
class SACTrainer(TorchTrainer):
    ...
    def classifier_init_training(self,  sim_replay_buffer,real_replay_buffer):
        ...
        self.trainX,trainY,valX,valY,testX,testY=get_data(sim_replay_buffer,real_replay_buffer)
        self.train_dataset =SAS_loader(trainX,trainY)
        self.train_dataloader=DataLoader(train_dataset,shuffle=False, batch_size=batch_size, drop_last=True)
        self.val_dataset =SAS_loader(valX,valY)
        self.val_dataloader=DataLoader(val_dataset,shuffle=False, batch_size=batch_size, drop_last=True)
        self.SAS_Network =   Classifier( Network=SAS_model ,
        ...
        self.SAS_Network.train(20)



def get_data(sim_memory,real_memory):
    #TODO: Change to compatible array
    print(sim_memory)
    X,Y=mixer(sim_memory, real_memory)

    len_data=len(Y)
    trainX=X[:len_data-20000]
    trainY=Y[:len_data-20000]
    valX=X[len_data-20000: len_data-10000]
    valY=Y[len_data-20000: len_data-10000]
    testX=X[len_data-10000:]
    testY=Y[len_data-10000:]
