...
import pybullet_data #Shreyas NOTE: Needs to go for other models
...
class MJCFBasedRobot(XmlBasedRobot):
  ...
  def reset(self, bullet_client):
    ...
    if (self.doneLoading == 0):
      ...
      if self.self_collision:
        ...
        #Shreyas edits: Making edits to load custom xml

        self.objects = self._p.loadMJCF(model_file,
                                        flags=pybullet.URDF_USE_SELF_COLLISION |
                                        pybullet.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS)
        if not self.is_real:
  ...
  def edit_xml(self, friction_value):
    replaced_str = 'friction="%1.1f %1.1f %1.1f"'%(friction_value[0], friction_value[1], friction_value[2])
    try:
      f = open(os.path.join(os.path.dirname(__file__),self.model_xml), 'r')
      data = f.read()
      model_xml = data.replace('friction=".8 .1 .1"', replaced_str)
      f.close()
      f = open(os.path.join(os.path.dirname(__file__), 'tmp', 'temp_model.xml'), 'w')
      f.write(model_xml)
      f.close()
    except:
