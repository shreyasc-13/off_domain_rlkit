...
class HalfCheetahEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, is_real): #params):
        ...
        if self.is_real:
            ...
            # np.random.seed(13)
            self.friction_value = np.random.normal([2, 0.5, 0.5], [2, 0.5, 0.5])
            # TODO: std dev <1 will have low variance

            model_xml = self._edit_xml(self.friction_value)
            mujoco_env.MujocoEnv.__init__(self, model_xml, 5)
    ...
    def reset_model(self):
        ...
        #For the sim_env
        #Shreyas TODO: May want to change implem
        if not self.is_real:
            self.friction_value = np.random.normal([2, 0.5, 0.5], [2, 0.5, 0.5])
            model_xml = self._edit_xml(self.friction_value)
            mujoco_env.MujocoEnv.__init__(self, model_xml, 5)
...
#Shreyas TODO: Needs to be fixed to be a gym env
